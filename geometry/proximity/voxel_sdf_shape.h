...
class VoxelSdfShape {
 public:
  ...
  bool supports_sampled_trilinear() const;
 ...
 private:
  ...
  static bool DoSupportsSampledTrilinear(const BoxData& box);

  static Sample DoEvaluate(const CylinderData& cylinder,
  ...
  static double DoCalcCharacteristicLength(const CylinderData& cylinder);
