...
DEFINE_double(duration, 0.0,
              "Simulation duration in seconds; 0 selects --duration_periods.");
DEFINE_double(settling_window, 0.0,
              "Final aggregation window in seconds; 0 selects "
              "--settling_window_periods.");
DEFINE_double(duration_periods, 15.0,
              "Duration in natural periods, used when --duration is 0. The "
              "body stops moving by about three periods, so this can be cut "
              "well below the default to buy sweep breadth.");
DEFINE_double(settling_window_periods, 1.25,
              "Aggregation window in natural periods, used when "
              "--settling_window is 0.");
DEFINE_double(target_equilibrium_penetration, 0.0,
              "Penetration in meters the derived load should produce; 0 uses "
              "the off-boundary default. Ignored when --mass is given. Use "
              "0.02 to place the plane on an affine cell boundary at every "
              "dyadic rung.");
...
DEFINE_int32(trajectory_stride, 1,
             "Write every Nth trajectory sample. A trajectory forces the "
             "surface and component pass at every written step, so striding "
             "is the cheap way to keep a long run affordable. Runs compared "
...
int DoMain(bool mass_was_set) {
  ...
  config.mass = mass_was_set ? FLAGS_mass
                             : DefaultSettlingMass(
                                   config.scene, config.hydroelastic_modulus,
                                   FLAGS_target_equilibrium_penetration > 0.0
                                       ? FLAGS_target_equilibrium_penetration
  ...
  config.duration_periods = FLAGS_duration_periods;
  config.settling_window_periods = FLAGS_settling_window_periods;
