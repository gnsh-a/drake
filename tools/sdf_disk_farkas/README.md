...
0.625 mm, a 0.5 ms time-step check at the finest grid, a dedicated affine-SDF
grid- and time-step-convergence sweep, and render the HTML report:

```bash
python3 tools/sdf_disk_farkas/run_comparison.py
```

Each simulation case writes a trajectory CSV plus companion
`<stem>_sap_stats.csv` and `<stem>_tamsi_stats.csv` solver-statistics files
(the SAP stats are populated under the lagged approximation, TAMSI under
tamsi). The report at `disk_farkas_affine_sdf.html` covers three questions:
SAP solver iterations (tet vs affine), trajectory error (affine vs the tet
control), and affine-SDF convergence under grid and time-step refinement.
...
that directory; it only overwrites its explicitly named CSV and HTML outputs.
