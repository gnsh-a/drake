...
int DoMain(int argc, char* argv[]) {
  ...
  // Four-DOF mount: {x, y, yaw} via a PlanarJoint on a massless carrier, plus
  // z via a PrismaticJoint from the carrier to the disk. Roll and pitch stay
  // locked (as with the plain PlanarJoint demo), but z is now free so the
  // normal load settles to N = mg dynamically instead of being fixed by a
  // pre-bisected penetration. The carrier is effectively massless: its
  // {x, y, yaw} DOFs are backed by the disk's inertia through the prismatic
  // joint, so the sliding mass and spin inertia stay exactly the disk's.
  const RigidBody<double>& carrier = plant.AddRigidBody(
      "disk_carrier", SpatialInertia<double>::SolidSphereWithMass(1e-9, 1e-3));
  const PlanarJoint<double>& disk_planar_joint = plant.AddJoint<PlanarJoint>(
      "disk_planar", plant.world_body(), RigidTransformd::Identity(), carrier,
      RigidTransformd::Identity(), Vector3d::Zero());
  const PrismaticJoint<double>& disk_lift_joint =
      plant.AddJoint<PrismaticJoint>("disk_lift", carrier,
                                     RigidTransformd::Identity(), disk_body,
  ...
  // Start at the scene's calibrated height with zero vertical velocity; the
  // disk then settles to the true N = mg penetration for this grid/modulus.
  disk_lift_joint.set_translation(&plant_context, p0.z());
  disk_lift_joint.set_translation_rate(&plant_context, v0.z());
