...
namespace drake {
namespace tools {
namespace sdf_disk_farkas {
namespace {

using Eigen::Vector2d;
using Eigen::Vector3d;
using geometry::AddCompliantHydroelasticProperties;
using geometry::AddCompliantHydroelasticVoxelSdfProperties;
...
Args ParseArgs(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (arg == "--settle-time") {
      args.settle_time = std::stod(ConsumeValue(&i, argc, argv));
    } else if (arg == "--quiet") {
      args.quiet = true;
    } else {
  ...
  if (args.settle_time < 0.0) {
    throw std::runtime_error("--settle-time must be non-negative.");
  }
  ...
  if (args.representation != "tet" && args.representation != "voxel_sdf") {
    throw std::runtime_error(
        "--representation must be one of tet or voxel_sdf.");
  }
...
ProximityProperties MakeProximityProperties(
    ...
    std::optional<double> relaxation_time, std::string_view representation) {
  ProximityProperties properties;
  if (representation == "tet") {
    AddCompliantHydroelasticProperties(resolution_hint, hydroelastic_modulus,
                                       &properties);
  } else {
    // Cylinder intentionally uses primitive-affine evaluation. In particular,
    // its top and bottom cap branches survive the one-cell disk thickness in
    // the canonical 2.5 mm Farkas scene.
...
Row SampleRow(const multibody::MultibodyPlant<double>& plant,
              ...
              double radius, const Context<double>& root_context, double time) {
...
void WriteRow(std::ofstream* out, const Row& row) {
  *out << row.time << "," << row.p_WD.x() << "," << row.p_WD.y() << ","
       ...
       << row.v_WD.z() << "," << row.angular_speed << "," << row.linear_speed
...
int DoMain(int argc, char* argv[]) {
  ...
  // No joint: the disk is a fully free 6-DOF body. Gravity and the compliant
  // hydroelastic contact alone hold it up and (if flat) keep it flat -- roll
  // and pitch are unconstrained, so this also tests whether the contact
  // moment self-stabilizes the disk rather than letting it tip.
  ...
  // Start flat at the calibrated height, at rest. The slide + spin are
  // imparted only after the settle phase (below).
  plant.SetFreeBodyPose(&plant_context, disk_body, RigidTransformd(p0));
  plant.SetFreeBodySpatialVelocity(
      &plant_context, disk_body,
      SpatialVelocity<double>(Vector3d::Zero(), Vector3d::Zero()));
  ...
  const int settle_frames =
      static_cast<int>(std::llround(args.settle_time * args.fps));
  ...
  WriteRow(&out, final);
  // Phase 1: settle from rest so the disk finds its flat, N = mg equilibrium
  // before any motion is imparted.
  ...
  // Impart the slide + spin as an instantaneous velocity kick (the Farkas
  // benchmark is defined by an initial velocity, not a ramped drive).
  {
    Context<double>& root_context = simulator.get_mutable_context();
    Context<double>& kick_context =
        diagram->GetMutableSubsystemContext(plant, &root_context);
    plant.SetFreeBodySpatialVelocity(&kick_context, disk_body,
                                     SpatialVelocity<double>(w0, v0));
  }
  // Phase 2: free deceleration under friction.
  for (int frame = settle_frames + 1;
       frame <= settle_frames + args.num_frames; ++frame) {
    const double time = frame * frame_dt;
    simulator.AdvanceTo(time);
    if (frame == settle_frames + 1 && args.surface_output.has_value()) {
...
int main(int argc, char* argv[]) {
  return drake::tools::sdf_disk_farkas::DoMain(argc, argv);
}
